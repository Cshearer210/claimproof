...
**Title:** Good first contribution: a stop-event adapter for AutoGen (or a runtime you actually use)
...
This one is AutoGen (or a runtime you actually use).
...
5. ****AutoGen ends a conversation through a termination condition, not a hook.** Find where a run genuinely finishes in the version you pin, and read a captured message rather than the class documentation.**
...
- **No new runtime dependency on the core library.** AutoGen (or a runtime you actually use) goes in an optional extra,
  exactly as `crewai` does in `pyproject.toml`.

### How to prove it

```
pip install -e '.[autogen]'   # add the extra in pyproject.toml, pinned

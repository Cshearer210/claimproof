...
**Title:** Good first contribution: a stop-event adapter for the OpenAI Agents SDK
...
This one is the OpenAI Agents SDK.
...
5. ****Output guardrails run on the final output only.** Confirm from the source whether yours sees tool-call history at all; if it does not, say so rather than inferring that work happened from the text.**
...
- **No new runtime dependency on the core library.** the OpenAI Agents SDK goes in an optional extra,
  exactly as `crewai` does in `pyproject.toml`.

### How to prove it

```
pip install -e '.[openai-agents]'   # add the extra in pyproject.toml, pinned

...
- **deadcanary: a `--baseline`/`--update-baseline` CI ratchet.** `--expect-dead N` demands
  ...
  wants answered: did coverage get WORSE than it has ever been recorded? No file at
  `PATH` yet means no baseline -- that run records one and passes, like a first commit.
  The file only ever ratchets down, and only with `--update-baseline`: a regressing run
  fails and never rewrites it, so one bad run cannot quietly relax the bar for the next.
- **deadcanary's own demo now has an animated SVG in its README**, generated by
  `tools/render_demo_svg.py deadcanary` -- the same live-run, refuses-to-drift renderer
  claimproof's README image already used.
- **deadcanary: `--verify-null`, a null-model check on every credited test.** A test
  earns credit for "catching" a corruption if it fails after the corruption is applied
  -- but a flaky test that fails on its own, unrelated to any corruption, would be
  credited the same way, and nothing was checking for that. `--verify-null` re-runs the
  suite `--null-repeats` times (default 2) with NO corruption applied and confirms every
  credited test stays green; anything that fails on its own is reported separately as
  `unreliable_killers` rather than silently trusted. Off by default -- it is real extra
  time on a real warehouse, proportional to how many tests actually caught something,
  never to the full corruption count. Additive only: it changes no existing verdict,
  count, or committed finding, which matters because three already-published results,
  including on dbt-labs' own template, rest on the numbers this does not touch.
- **`Cshearer210/deadcanary`'s social preview card.** `packages/deadcanary/tools/social_card.html`
  is the source (plain HTML/CSS, no build step, regenerable by any headless browser);
  `assets/social-card.png` is the 1280x640 image GitHub actually serves when the repo
  link is shared. Its one quoted stat is pulled verbatim from README.md's own findings.
- **A GitHub Action**, `packages/deadcanary/action.yml`. Wraps the `--baseline`/
  `--update-baseline` ratchet above in three lines of workflow YAML
  (`uses: Cshearer210/claimproof/packages/deadcanary@main`). Deliberately does not commit
  an improved baseline back to the calling repo -- a composite action with write access
...
- **`tools/necessary_files.py`'s allowlist did not cover a package's own `assets/`
  folder**, so claimproof's own demo image was allowed only because it happens to sit
  at the repo root -- the same image one level down, inside `packages/deadcanary/`,
  would have been refused by CI the moment it was committed. `packages/[^/]+/assets/`
  is now allowed alongside the existing `src`/`tests`/`tools`/`findings` subfolders, and
  `action.yml` alongside the other named package-root files.
- **`--json` did not actually mean "stdout is nothing but the report."** `--attest`,
  `--expect-dead` and the new `--baseline` each print a plain-English status line on
  success, unconditionally, on stdout -- harmless alone, and silently wrong the moment
  `--json` was also passed, which is exactly the combination the new GitHub Action relies
  on to read the dead-canary count back out. Proved the bug was real before fixing it:
  reproduced the exact "extra data" JSON parse failure the old code produced, confirmed
  the fix parses clean, and added 4 tests covering every affected flag combination plus a
  guard that plain (non-JSON) mode still prints to stdout as before.

### Changed
- **`tools/render_demo_svg.py`'s act-marker guard is no longer hardcoded to claimproof's
  numbered acts.** It refused to render anything without lines starting "1. " through
  "5. ", which is specific to claimproof's demo and meaningless for deadcanary's, which
  has no acts. `demo_lines()` now takes a `start_marker`/`end_marker` line-prefix pair
  per demo instead, with claimproof's five-act values as the default -- verified
  byte-identical against the prior asset by hashing the output, not by reasoning that a
  refactor is safe.
